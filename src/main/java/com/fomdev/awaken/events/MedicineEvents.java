...
public class MedicineEvents
{
    ...
    )
    {
        ...
        int value = (int) Math.pow(proficiency, 1.0 / 4.0);
        if (value <= 1)
            return;

        int count = random.nextInt(8);

        Records.AwakenMedicineComponent medicine = new Records.AwakenMedicineComponent(spore.getLocation().toString(), value);
